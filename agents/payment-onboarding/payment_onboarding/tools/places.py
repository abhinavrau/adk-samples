...
def get_photo_url(photos_data: List[Dict[str, Any]], api_key: str, max_width: int = 400) -> str:
    """
    Constructs a photo URL from the first photo reference.
    """
    if not photos_data or not isinstance(photos_data, list) or not photos_data[0].get("photo_reference"):
        return ""
...
def get_place_details(place_id: str, api_key: str) -> Union[Dict[str, Any], Dict[str, str]]:
    """
    ...
    fields = [
        ...
        "rating", "editorial_summary", "photo",
        "business_status"
    ]
    params = {
        "place_id": place_id,
    ...
    try:
        ...
        if status == "OK":
            ...
            error_message = details_data.get("error_message", f"Place Details API Error: {status}")
            return {"error": error_message, "status": status}

    except requests.exceptions.RequestException as e:
        return {"error": f"Network error fetching place details: {e}"}
    except Exception as e:
        return {"error": f"An unexpected error occurred during place details fetch: {e}"}

# --- Modified Function: Find Businesses From Text ---
def find_businesses_from_text(
    ...
    try:
        ...
        if search_status == "ZERO_RESULTS":
            ...
            error_message = search_data.get("error_message", f"Text Search API Error: {search_status}")
            return {"error": error_message, "status": search_status}

        place_ids_from_search = [place.get("place_id") for place in search_data.get("results", []) if place.get("place_id")]

        for p_id in place_ids_from_search[:max_results]: # Use p_id to avoid conflict with details['place_id']
            ...
            if "error" not in details:
                ...
                highlights = ""
                if details.get("editorial_summary") and isinstance(details["editorial_summary"], dict):
                    highlights = details["editorial_summary"].get("overview", "")
                
                image_url = get_photo_url(details.get("photos", []), api_key)
                
                actual_place_id = details.get("place_id", "") # Get the actual place_id from details
                map_url = f"https://www.google.com/maps/place/?q=place_id:{actual_place_id}" if actual_place_id else ""
                ...
                print(f"Warning: Could not get details for place_id {p_id}: {details.get('error')}")
